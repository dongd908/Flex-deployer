...
bool SearchGraph(Graph &ori_graph, Graph &sub_graph, int32_t K) {
	graphStatus ret = GRAPH_FAILED;
	std::vector<Graph*> altersubgraph;
	
	std::vector<GNode> nodes = ori_graph.GetAllNodes();
	for (auto &node : nodes) {
	   // transfer GNode to ge::Operator
        ge::AscendString name; // get op name
        ret = node.GetName(name);
        if (ret != GRAPH_SUCCESS) {
            std::cout<<"Get node name failed."<<std::endl;
            return false;
        ...
        ge::Operator op_node(name, type); // set op name & type
        ret = sub_graph.AddOp(op_node);
        if (ret != GRAPH_SUCCESS) {
            std::cout<<"add node to sub_graph failed."<<std::endl;
            return false;
        }
        
        if (node_type != "Const") { // set op input
            std::vector<ge::AscendString> input_name_key; 
            ret = node.GetAttr("_input_name_key", input_name_key);
            if (ret != GRAPH_SUCCESS) {
                std::cout<<"Get input name key failed."<<std::endl;
                return false;
            }
            for (int32_t i = 0;i < node.GetInputsSize();i++) {
        	      std::pair<std::shared_ptr<GNode>, int32_t> src_tensor_info = node.GetInDataNodesAndPortIndexs(i);
        	      GNode src_node = *(src_tensor_info.first);
        	      ge::AscendString src_name;
        	      ret = src_node.GetName(src_name);
        	      ge::Operator op_src;
        	      ret = sub_graph.FindOpByName(src_name.GetString(), op_src);
        	      if (ret != GRAPH_SUCCESS) {
            	      std::cout<<"find src node by name failed."<<std::endl;
                     return false;
                }
        	  	 op_node.SetInput(input_name_key[i].GetString(), op_src, src_tensor_info.second);
            }
        }
...
bool GenSubGraph(Graph &ori_graph, Graph &sub_graph) {
	graphStatus ret = GRAPH_FAILED;
	std::vector<Graph*> altersubgraph;
	...
	for (auto &node : nodes) {
	   // transfer GNode to ge::Operator
        ...
        if (node_type != "Const") { // set op input
        	  std::cout<<"node type is not Const"<<std::endl;
            std::vector<ge::AscendString> input_name_key; 
            if (!node.HasAttr("_input_name_key")) {
            	std::cout<<"attrubite _input_name_key is not exist"<<std::endl;
            }
            if (!node.HasAttr("input_name_key")) {
            	std::cout<<"attrubite input_name_key is not exist"<<std::endl;
            }
            if (!node.HasAttr("id")) {
            	std::cout<<"attrubite id is not exist"<<std::endl;
            }
            /*
            for (int32_t i = 0;i < node.GetInputsSize();i++) {
        	      std::pair<std::shared_ptr<GNode>, int32_t> src_tensor_info = node.GetInDataNodesAndPortIndexs(i);
...
int main(int argc, char* argv[])
{
    ...
    Graph sub_graph("subgraph0");
    ret = GenSubGraph(graph1, sub_graph);
    if (!ret) {
    		cout << "========== Generate subgraph Failed! ==========" << endl;
            return -1;
    } else {
            cout << "========== Generate subgraph Success! ==========" << endl;
    }
    ...
    cout << "BuildInitialize Success." << endl;
    
    // E. dump graph
    /*size_t filesize = 26;
    const char* file = "ge_gengraph_before_nesting";
    status = aclgrphDumpGraph(graph1, file, filesize);
    if(status != GRAPH_SUCCESS) {
        cout<<"Dump Graph Faied."<< endl;
    }	else {
        cout<<"Dump Graph Success." << endl;
    }*/
    size_t filesize = 12;
    const char* file = "gen_subgraph";
    status = aclgrphDumpGraph(sub_graph, file, filesize);
    if(status != GRAPH_SUCCESS) {
        cout<<"Dump Graph Faied."<< endl;
    }	else {
        cout<<"Dump Graph Success." << endl;
